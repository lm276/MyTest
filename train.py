...
class UCGL(nn.Module):
    def __init__(self, n_in, n_h, nb_classes, n_clusters, pretrain_path='pretrained_model_pubmed.pkl'):  # pretrain_path should be added
    ...
    def pretrain(self, adj, diff, features, labels, idx_train, idx_val, idx_test):
        self.mvg.load_state_dict(torch.load(self.pretrain_path,map_location=torch.device('cpu')),strict=False)
...
def train_ucgl(dataset):
    ...
    #     labels = labels.cuda()
        ...
        # diff = torch.from_numpy(diff).cuda()
        # idx_train = torch.from_numpy(idx_train).cuda()
        # idx_val = torch.from_numpy(idx_val).cuda()
        # idx_test = torch.from_numpy(idx_test).cuda()
        # import scipy.sparse as sp
        # sp_adj = sp.coo_matrix(adj.all())
        # values = sp_adj.data
        ...
        # shape = sp_adj.shape
        # adj_tensor = torch.sparse.FloatTensor(i, v, torch.Size(shape)).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain_path', default='pretrained_model_citeseer.pkl', type=str,
    ...
    parser.add_argument('--mask_num', default=3,type=int)
    ...
    if args.dataset == 'cora':
        ...
        args.n_clusters = 6
    args.model_path = os.path.join('D:\AI_train\Contrastive_Learning\GDCL\model_path/UCGL_' + args.dataset + '_' + str(args.hid_units) + '.pkl')
